 ...
 */
var topLevelTypes = keyMirror_1$1({
    topAbort: null,
 ...
 * of patent rights can be found in the PATENTS file in the same directory.
 *
 * Module getVendorPrefixedEventName
 */
 ...
 * @internal
 */
var ReactBrowserEventEmitter = index$1({}, ReactEventEmitterMixin, {
...
function diff(dom, vnode, context, mountAll, parent, componentRoot, component) {
    ...
    var nextSibling = dom && dom.nextSibling,
...
function idiff(dom, vnode, context, mountAll, inst) {
    ...
    var prevChildren = inst && inst._renderedChildren;
    ...
    // flat children
    var childrenFlated = vnode && vnode.children != null && flatChildren(vnode.children);
...
function innerDiffNode(dom, childrenFlated, context, mountAll, absorb, childrenPrevflated) {
    var originalChildren = dom.childNodes,
        originalChildrenArr = [].slice.call(originalChildren, 0),
        // array never changes
    ...
    if (vchildren != null) {
        ...
        for (key in vchildren) {
            ...
            // new mount index
            var newIndex = mountIndexes ? mountIndexes[key] : index;
            // old mount index, first render !! eg: server render
            var prevIndex = prevMountIndexes ? prevMountIndexes[key] : index;

            prevChildNode = originalChildrenArr[prevIndex];
            var recyclable = prevChildNode && isSameNodeType(prevChildNode, vchild, hydrating);
...
function recollectNodeTree(node) {
    var unmountOnly = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    ...
    if (component) {
        ...
        // m-start
        // qreact nodes
        var attributes = node[ATTR_KEY] && node[ATTR_KEY];
 ...
 */
var __keyed__ = '__keyed__';
function flatChildren(children) {
    var ret = _flatChildren(children, '.', {
        __counter__: 0
    }, {}),
...
function _flatChildren(children, prefix, childrenFlated, existKeys, index) {
    ...
    if (singleNode) {
        var key = children.attributes && children.attributes.key;
        if (key != null) {
        ...
        var mountIndexes = childrenFlated[__keyed__] = childrenFlated[__keyed__] || {};
        mountIndexes[key] = childrenFlated.__counter__++;
    } else {
        var _prefix = prefix + (index != null ? index + ':' : '');
        existKeys = {};
...
function renderComponent(component, opts, mountAll, isChild) {
    ...
    if (!skip) {
        ...
        if (typeof childComponent === 'function') {
            ...
            if (initialBase || opts === SYNC_RENDER) {
                ...
                // m-start
                base = diff(cbase, rendered, context, mountAll || !isUpdate, initialBase && initialBase.parentNode, true, component);
                // m-end
            }
        }
    ...
    if (!isUpdate || mountAll) {
        ...
        // m-start
        mounts.unshift([component, previousProps, previousState, previousContext]);
        // m-end
    }
...
function unmountComponent(component) {
    ...
    component.base = null;
    // m-start
    base._component = null; // if not recycle component
    // m-end
    ...
    if (inner) {
        ...
        // m-start
        component.nextBase = base; // ? memory leak
...
function render$1(vnode, parent, merge) {
    ...
    var ret = diff(merge, vnode, {}, false, parent, !'componentRoot', component);
    return ret;
}
 ...
 * of patent rights can be found in the PATENTS file in the same directory.
 *
 * Module SyntheticTransitionEvent
 */
 ...
 * of patent rights can be found in the PATENTS file in the same directory.
 *
 * Module SyntheticAnimationEvent
 */
...
// polly fill
Object.assign = Object.assign || index$1;
...
function cloneElement(element) {
    ...
    var newProps = extend(extend({}, elementProps), props);
    var c = void 0;
...
function shallowDiffers(a, b) {
    for (var _i2 in a) {
        if (!(_i2 in b)) return true;
    }for (var _i3 in b) {
        if (a[_i3] !== b[_i3]) return true;
    }return false;
}
...
function collateMixins(mixins) {
    var keyed = {};
    for (var _i4 = 0; _i4 < mixins.length; _i4++) {
        var mixin = mixins[_i4];
...
function bindAll(ctx) {
    for (var _i5 in ctx) {
        var v = ctx[_i5];
        if (typeof v === 'function' && !v.__bound && !AUTOBIND_BLACKLIST.hasOwnProperty(_i5)) {
            (ctx[_i5] = v.bind(ctx)).__bound = true;
        }
    }
}
...
function multihook(hooks, mergeFn) {
    return function () {
        var ret = void 0;
        for (var _i6 = 0; _i6 < hooks.length; _i6++) {
            var r = callMethod(this, hooks[_i6], arguments);
...
extend(Component$$1.prototype, {
    ...
    replaceState: function replaceState(state, callback) {
        ...
        for (var _i7 in this.state) {
            if (!(_i7 in state)) {
                delete this.state[_i7];
            }
        }
    },
...
var qreact = {
    version: version,
    options: options, // support devtools
    h: h,
    DOM: DOM,
    PropTypes: PropTypes, // actually proptypes target in src/proptypes.js
    Children: Children,
    render: render$$1,
...
//# sourceMappingURL=qreact.js.map
